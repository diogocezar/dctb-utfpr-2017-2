Gulp = {
	...
	imgmin: function(){
		Gulp.self.task('imgmin', function(){
			...
			Gulp.self.src('./images/**/*')
			.pipe(Gulp.imagemin())
			.pipe(Gulp.self.dest('./images/'));
		});
	},
	svgopt: function(){
		Gulp.self.task('svgopt', function(){
			console.log('[SVGO] Otimizando arquivos SVG');
			Gulp.self.src('./images/**/*.svg')
			.pipe(Gulp.svgo())
			.pipe(Gulp.self.dest('./images'));
		});
	},
	scss: function(){
		Gulp.self.task('scss', function(){
			...
			Gulp.self.src('./scss/base/base.scss')
			.pipe(Gulp.sass())
			.pipe(Gulp.self.dest('./css/src/base'));

			Gulp.self.src('./scss/pages/**/*.scss')
			.pipe(Gulp.sass())
			.pipe(Gulp.self.dest('./css/src/pages'));
		});
	},
	css: function(){
	...
	watch: function(){
		Gulp.self.task('watch', function(){
			...
			Gulp.self.watch('./scss/**', ['scss']);
			Gulp.self.watch('./images/**/*.svg', ['svgo']);
		});
	},
	wsass: function(){
